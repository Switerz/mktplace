...
**Frente Oráculo (conector MCP) — 21/08/2026: `OM2 TASK 1/2 CONCLUÍDA — OAUTH IMPLEMENTADO LOCALMENTE, NÃO PUBLICADO`.**
Estado por gate: **OM0 concluído**; **OM1 implementado, versionado e validado localmente**;
**OM2-P0 concluído** (`NO-GO` para bearer estático no Claude.ai); **OM2 Task 1/2 concluída —
OAuth 2.1 com Auth0 implementado e validado localmente**; **OM2 Task 2/2 (publicação) não
iniciada**. O bloqueio de identidade foi **resolvido**: o proprietário criou o tenant Auth0
...
públicos. **Suíte completa 100% verde (986 testes, 241 do MCP)**, typecheck e build limpos;
uma dependência de runtime adicionada (`jose`, sem vulnerabilidade, com mudança de **uma
linha** no lockfile). **Nada foi publicado:** zero deploy, zero variável na Vercel, zero
segredo criado, Claude.ai **não** conectado. **Próximo passo: OM2 Task 2/2** — configurar as
variáveis públicas na Vercel e informar Client ID/Secret diretamente no Claude.ai.
