...
| Revamp UI/UX (Torre) | **U2 CONCLUÍDO — PRONTO PARA U3** | Gate U0 concluído em 24/07 ([UI_REVAMP_PLAN.md](UI_REVAMP_PLAN.md)). Gate U1 (fundação visual e novo shell) concluído e aprovado em 24/07: sidebar clara/lavanda persistente no desktop, drawer mobile (com focus trap) e topbar compartilhados substituem o antigo `AppNav` por página. Gate U2 (Gerencial completa e padrão de drill-down) concluído em 24/07: Gerencial reorganizada (cabeçalho, filtros, resumo executivo, KPIs, tendência + painel de canais, tabela por marca); 4 KPIs com drill-down agregado acessível num diálogo reutilizável; navegação preserva filtros globais e respeita precedência do destino. Filtros globais, contratos de API e regras de negócio preservados em ambos os gates; typecheck/build/testes passando. | Gate U3 — Canais e Marcas. | Nenhuma dependência bloqueante. |
