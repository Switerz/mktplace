...
**Última atualização:** 27/08/2026 — **Frente Oráculo: OM2 Task 1/2 VERSIONADA E PUBLICADA (`7b1b451`), OAuth ATIVO em produção e comprovado por smoke read-only; o CLAUDE.AI ESTÁ CONECTADO em produção. O gate OM2-C falsificou a hipótese de incompatibilidade de protocolo — o `/api/mcp` já atendia o ciclo 2025-06-18 (initialize → initialized → tools/list → tools/call), provado com cliente MCP oficial sobre HTTP real —, e o OM2-E versionou em `7c5aab5` a observabilidade sanitizada que classificou a fronteira real da falha: 401 `bearer_missing` no discovery, depois 403 `insufficient_scope`, isolando AUTORIZAÇÃO. O proprietário corrigiu a configuração de autorização/scope no Auth0 e a conexão passou a funcionar; a skill `/oraculo` usou a integração e o conector enumerou exatamente cinco tools sobre a camada `marts` no Neon. Nenhuma alteração de protocolo, pacote ou lockfile foi feita. PENDENTE: o smoke funcional — uma chamada real por tool — ainda NÃO foi executado, e o acesso continua dependendo de role manual enquanto a Post-Login Action do OM3 — versionada em `588b046` — não for publicada no Auth0. Suíte 1144/1144 e MCP 316/316 verdes, typecheck e build limpos. Contexto anterior (superado): o OM2-P0 havia concluído NO-GO para bearer compartilhado — o custom connector do Claude.ai não aceita header estático — e a frente estava BLOCKED_BY_IDENTITY_PROVIDER aguardando a escolha de um issuer; esse bloqueio foi resolvido com o Auth0. Frente Gate V3: V3-0, V3-1A, PF1, V3-BE e V3-1B VERSIONADOS (`2d7ecdf` fecha o V3-1B, com QA visual final de 96/96), contrato do V3-BE CONFIRMADO EM PRODUÇÃO; V3-2 (Marca 360) e V3-3 (QA visual integrado) TECNICAMENTE CONCLUÍDOS E VERSIONADOS; GATE V3 ENCERRADO COMO `PASS` EM 25/08/2026: o smoke de produção havia sido `PASS WITH ISSUE`, os três achados cosméticos foram corrigidos em `84c9d5f`, as duas pendências terminais saíram em `a893a2b` (contagens exatas do drill-down) e `aa6e245` (foco nas transições internas da matriz), e o SMOKE TERMINAL APROVOU depois de o proprietário REPUBLICAR MANUALMENTE O BACKEND no Render — a primeira tentativa foi `NO-GO` só porque o `reference_note` chegava sem acentuação de uma revisão anterior a `84c9d5f`; no reteste, um GET no endpoint devolveu o texto acentuado exato e a confirmação visual focal mediu 31/31 nos dois viewports. Dívidas de contraste (gray-on-color) e o 422 do `resolve_brands` PERMANECEM ABERTAS e fora do `PASS`; **GATE UE3 CONCLUÍDO, VERSIONADO E PUBLICADO** — implementação, testes e QA integrada versionados em 27/08/2026 (`8760f96`); **backend publicado manualmente pelo proprietário no Render** e **frontend publicado pelo fluxo automático da Vercel**; **smoke de produção `PASS WITH ISSUE`**, com comportamento compatível com `8760f96` e **mediana de 423 ms** no cenário com o bloco — a diferença entre cenários **não é atribuível causalmente ao bloco** e o **RTT interno Render→Neon NÃO foi medido**; ressalva de um **404 intermitente, sem URL, não reproduzido em seis tentativas e não atribuível a esta frente** (ver a seção do Gate UE3 abaixo): QA integrado em navegador real nos três viewports, QA integrado em `PASS`: 402/414 verificações aprovadas depois do patch terminal de acessibilidade compartilhada, que fechou as 12 reprovações de alvo <44px e tick <12px medindo em 9 rotas × 3 viewports; base integrada até `76f361b`; TODAS as fases do Gate V3 fechadas tecnicamente; Gate S3 é checkpoint funcional publicado com observação agendada ESTACIONADA.** O S3 foi validado como `PASS WITH ISSUE` no smoke pós-deploy: `/inteligencia` e `/brand-detail` respondem **200 em produção** (2,3 s e 0,5–1,0 s) lendo `marts.*`, contra 500 em ~10,4 s antes; as quatro cargas iniciais e a reconciliação foram aprovadas. Duas pendências seguem abertas e **não** fazem parte do V3: o smoke visual do dashboard **pós-publicação do S3** (sem driver de navegador instalado no ambiente daquele gate — não tem relação com o QA visual local do V3-1A, que **foi executado**) e a **observação da primeira execução agendada do `full_daily` com os 12 steps**, que ficou estacionada. A frente **Gate V3 — Inteligência e Marca 360** está com o **V3-0, o V3-1A, o PF1, o V3-BE e o V3-1B versionados (`309b6bf`, `e675948`, `45fa3f8`, `26434c8`, `2d7ecdf`), o contrato do V3-BE confirmado em produção, e o **V3-2 (Marca 360) e o V3-3 (QA visual integrado) tecnicamente concluídos e versionados no commit de fechamento do Gate V3, com publicação e smoke pós-deploy ainda pendentes**. O desenho do V3-0 continua sendo a base: a auditoria da referência `torre_b2b` e das duas superfícies foi concluída e uma rodada de correção terminal reescreveu sete contratos do desenho. Os dois mais materiais: a **matriz 2×2 de oportunidades não era implementável com o payload daquele momento** (achado de 18/08, hoje resolvido pelo BE6) — `urgent ∪ scale ∪ organic` é amostra capada, deixa de fora `sells+advertised` com ROAS < 8 e todo `inactive`, e **não existe corte de GMV** (`product_status` distingue venda/Ads, não volume) —, passando a depender do contrato aditivo **BE6 `opportunity_map`**; e a **lista de competências reais virou bloqueio do V3-2** (**BE5 `available_months`**), porque `/brand-detail` devolve só o mês pedido e a lista **não é derivável** da resposta. Plano em [INTELIGENCIA_BRAND_V3_PLAN.md](INTELIGENCIA_BRAND_V3_PLAN.md), com roadmap V3-0 desenho → V3-1A fundação (usa o payload atual) → contrato backend aditivo → V3-1B matriz definitiva → V3-2 Marca (exige BE5) → V3-3 QA. **O desenho foi aprovado e versionado no V3-0, que não criou código, endpoint, rota ou dependência.** O **V3-1A foi implementado depois** sobre o payload atual, teve **QA visual executado em navegador real** e teve os quatro achados de alvo/tipografia (**A1–A4**) **fechados**; nenhuma alteração de backend foi feita em nenhuma das duas etapas. **O gate de plataforma PF1 foi entregue e versionado em `45fa3f8`**: `withCache` deixou de memoizar respostas degradadas e "Tentar novamente" voltou a consultar a rede. Os contratos backend também saíram: **BE3, BE4, BE5 e BE6 foram versionados no V3-BE (`26434c8`), com o contrato confirmado em produção**, o que **desbloqueou a matriz definitiva do V3-1B e o V3-2**. **Próximo marco: a revisão e o versionamento do V3-2 + V3-3 em conjunto**, que fecham a frente inteira do Gate V3. Nenhuma fase do V3 segue aberta tecnicamente.** Contexto imediatamente anterior — **Gate S3 Task 3/3: `SUCCESS` na publicacao dos dados; backend ainda dependente de deploy manual do Render.** Migrations **009/010/011 aplicadas** no Neon (`008 -> 011`, uma tentativa, zero relacao preexistente alterada) e as **quatro cargas concluidas**: ML produto 1.648/1.648, TikTok produto full 214.573/214.573, `ml_cross_company` 4 e `tiktok_channel_efficiency` 4.743 — `EXCEPT` bidirecional `(0,0)` nas quatro e fingerprint identico. A **primeira** tentativa de ML reprovou por guardrail (`staging divergiu da fotografia no fingerprint`) com **rollback integral provado** e ficou registrada como `170 failed`: a Gold usa `NUMERIC` sem escala e o destino declara escala, entao a staging (`LIKE` do destino) arredondava e o codigo comparava valor cru contra valor arredondado — 1.388 de 1.648 linhas so' em `cumulative_revenue_pct`. Corrigido em `3559ab7` com projecao canonica para os tipos do destino, lida do `information_schema` e com `ROUND_HALF_UP` conferido contra o banco real em 21 casos (zero divergencia); o guardrail **nao foi afrouxado**. **`/inteligencia` e `/brand-detail` respondem 200 com o Data Mart indisponivel e zero query roteada para ele** — antes eram 500 —, mas **somente em execucao local**: o Render nao publicou o S3. `/produtos` ML sem regressao nas 8 chamadas do baseline (schema, campos, paginacao e contagens identicos). Health check com `STATUS CRITICO: OK` e as duas fontes novas frescas. Isolamento: 37 de 39 tabelas de `marts` com contagem identica, zero relacao fora de escopo alterada. **Concorrencia observada:** houve uma segunda execucao completa em 18/08 as 11:10–11:33 por **invocacao direta**, fora do `run_with_lock` e sem rastro em `logs/` — divida de coordenacao registrada em §39.2. **A execucao agendada do `full_daily` com os 12 steps ainda nao foi observada** (primeira oportunidade: 19/08 06:00). **Airflow continua inexistente.** Detalhes em §39. Contexto imediatamente anterior — **Gate S3 Task 2/3 implementada em worktree isolado.** `/inteligencia` e `/brand-detail` passaram a ler `marts.*` no codigo, com os dois contratos de payload congelados ANTES da troca e **verdes nos dois estados sem uma unica expectativa editada**. Tres migrations novas (009/010/011) escritas e **nao executadas**; dois syncs de snapshot proprios (o wrapper do O1 nao aceita snapshot e nao foi generalizado); `sync_ml` virou snapshot transacional real, corrigindo o "full refresh" que era apenas upsert. `full_daily` foi para **12 steps** (orcamento 7.500s contra 9.000s). **Nada foi aplicado:** zero migration, zero escrita em banco, zero backfill, zero Scheduler, zero commit/push/deploy — e o **checkout operacional continua byte a byte intacto** para a execucao de 19/08 as 06:00. `/inteligencia` e `/brand-detail` **seguem 500 em producao**; `/tempo-real` fora do S3. O `health_check` passou a monitorar as duas fontes novas — execucao (30h, criticas) e cobertura (`MAX(synced_at)` e `MAX(date)`) —, fechando a lacuna em que um sync podia falhar por dias com `ok_critical=true`. **Airflow nao e' necessario neste gate.** A ordem obrigatoria da Task 3 esta em §38.12, e o **primeiro passo dela mudou de estado**: `origin/main` avancou para `a1c5ffe` (*fix(pipelines): impede efeitos colaterais no help regional*, 18/08), entao o hotfix **ja esta publicado** e a base do worktree S3 (`d04306e`) esta **um commit atras**. O hotfix toca somente `pipelines/ops/sync_region_if_needed.py` e seu teste — **zero conflito** com os 25 arquivos do S3. A incorporacao nao foi feita nesta rodada. Contexto imediatamente anterior — 17/08/2026 — **Checkpoint O1: ponte de serving versionada e PILOTO EXECUTADO com `STATUS GERAL: OK`.** Codigo em `950c407` (11 arquivos) e uma unica invocacao da TaskKey manual `serving_refresh` em 12:33:16..12:34:21 (65,1 s), **zero retry**: `serving_ml` 360/360, `serving_tiktok_brand` 450/450 e `serving_tiktok_creator` 65.574 apagadas / **66.223** publicadas, `EXCEPT` bidirecional (0,0) nos tres. Os tres watermarks efetivos ficaram em **16/08 (D-1)** — o ML tinha D0 na Gold e foi cortado corretamente. Reconciliacao com **zero delta pos-snapshot**; `/operacoes` em producao responde **200 em 0,45 s** com payload **byte-identico** ao Neon atualizado e zero dado de D0. **A `serving_refresh` esta operacional; a primeira execucao AGENDADA do `full_daily` com os novos steps ainda nao foi observada.** **Nenhuma alteracao do Scheduler**, que segue habilitado com proxima execucao em 18/08 06:00. **Airflow nao existe.** Confiabilidade segue dependente de notebook ligado, usuario logado e VPN; **06:00 permanece divida**; backfill periodico ainda pendente. Contexto imediatamente anterior — **Checkpoint O1 Task 2/2: ponte de serving `IMPLEMENTADA E TESTADA, NAO EXECUTADA`.** Os tres syncs de serving passaram a existir como steps do `full_daily` (`serving_ml`, `serving_tiktok_brand`, `serving_tiktok_creator`), todos `critical=True`, cada um resolvendo a propria janela por `min(D-1, source_max)` — creator em D-2 nao rebaixa ML nem brand. **Nada rodou:** nenhum sync foi executado, nenhum banco escrito, **nenhum Scheduler criado ou alterado** e nenhum deploy feito. Afirmar que o serving ja se atualiza em producao seria falso. **Airflow continua inexistente.** A confiabilidade segue dependente de notebook ligado, usuario logado e VPN ativa, e o horario **06:00 continua divida** — em agosto so' 5 dos 17 dias tiveram execucao as 06:00. A TaskKey `serving_refresh` e' **contingencia manual**, nao agendada. Conclusao operacional depende de **piloto autorizado**. Contexto imediatamente anterior — **Gate S2 FECHADO: `PASS COM RESTRIÇÃO`. `/operacoes` está em produção lendo o Neon** (revisão `41eb171`, publicada manualmente no Render pelo proprietário): HTTP 200 em ~0,45 s, contra 500 em ~10,4 s antes. Serving cobre ML e TikTok brand até 16/08 e TikTok creator até 15/08 — 16/08 do creator é **ausência da fonte, nunca zero fabricado**. **O Gate G4 NÃO foi fechado:** `/inteligência`, `/tempo-real` e `/brand-detail` seguem 500 por dependência do Data Mart e são escopo do **Gate S3, não iniciado**. **Airflow continua inexistente e não comprovado**, e os syncs de serving **não estão no `full_daily`** — sem execução manual o serving volta a defasar. (ver **Checkpoint 17/08/2026** logo abaixo do resumo executivo). Contexto anterior — 11/08/2026 (**Frente ativa: camada de serving Data Mart → Neon — Gate S1 encerrado como `PARTIAL — PILOTO VALIDADO`**: migration `006` aplicada e `marts.fact_ml_gestao_diaria` criada, carregada e reconciliada; **execução dentro de worker Airflow ainda não comprovada** — é a razão do `PARTIAL`; **Gate S2 Task 3/3: `SUCCESS — BACKFILL COMPLETO E /OPERACOES PRONTO PARA VERSIONAMENTO`** — as três fatos de serving recarregadas até D-1 (12/08/2026) e reconciliadas, `/operacoes` com payload idêntico ao da Gold e provado sem Data Mart. **Ainda não versionado nem publicado.** Sanitização de erro endurecida nos dois módulos de serving: topologia não vaza mais em log. As **quatro** superfícies do G4 respondem 500 em produção — a medição de 10/08 que indicava três estava errada por janela de espera curta. **Revamp Visual V2 encerrado: `PUBLICADO — PASS WITH ISSUE`.** Fechamento publicado no commit `04d0d17` e validado por smoke read-only em 10/08/2026: 11 rotas em HTTP 200, sticky da Gerencial em `top=0px` nos três viewports, zero overflow, ticks semanais legíveis, comparação e diálogo aprovados, **zero regressão causada pelo release**. Restrição operacional aberta: as **quatro** superfícies do G4 (`/brand-detail`, `/tempo-real`, `/inteligencia`, `/operacoes`) sem fonte em produção — dependem do Data Mart, inalcançável do Render. Frente do Revamp **encerrada**; V2-0 a V2-4 em `c110e85`, `13c7ee0`, `e8f0630`, `2336567` e `04d0d17`.)
...
**Frente Oráculo (conector MCP) — 25/08/2026: `CONECTADO EM PRODUÇÃO; SMOKE FUNCIONAL DAS CINCO TOOLS AINDA PENDENTE`.**
Estado por gate: **OM0 concluído**; **OM1 implementado, versionado e validado localmente**;
**OM2-P0 concluído** (`NO-GO` para bearer estático no Claude.ai); **OM2 Task 1/2 versionada e
publicada em `7b1b451`**; **OM2 Task 2/2 Fase A concluída** — o proprietário configurou as cinco
variáveis em Production, e o smoke read-only de 21/08 mediu **200 nas duas rotas de Protected
Resource Metadata** (corpos byte-idênticos, `resource`/`authorization_servers`/`oracle:read`
corretos) e **401 com `WWW-Authenticate` bem-formado** em `GET`/`POST`/`DELETE` sem bearer, com
Auth0 discovery e JWKS (2 chaves RSA/RS256) alcançáveis. **OM2-E versionado em `7c5aab5`**;
**OM3 versionado no commit de fechamento deste gate; a Action ainda NÃO foi publicada no Auth0**.

**RESOLVIDO — a conexão foi estabelecida.** A observabilidade sanitizada do OM2-E cumpriu o papel
para que foi feita: os eventos classificaram a fronteira em vez de exigir adivinhação. A sequência
observada foi **401 `bearer_missing`** — esperado durante o discovery, quando o cliente ainda não
tem credencial — e depois **403 `insufficient_scope`**, que apontou diretamente para autorização, e
não para protocolo nem para ambiente. O proprietário corrigiu a configuração de
autorização/scope no Auth0 e **o Claude.ai conectou sem erro**. Confirmado pelo proprietário: a
skill `/oraculo` usou explicitamente a integração "Oráculo — Torre de Marketplaces", o conector
identificou a camada `marts` no Neon e **exatamente cinco tools foram enumeradas**.

**O que isso NÃO prova.** Enumerar cinco tools é inventário, não execução: **o smoke funcional
— uma chamada real por tool — ainda não foi executado**. E o acesso **continua dependendo de
role/autorização manual** enquanto a Post-Login Action do OM3 não for publicada.

| Resolvido | Ainda pendente |
|---|---|
| OAuth/Auth0 ponta a ponta | publicação manual da Post-Login Action v3 no Auth0 |
| Emissão de bearer válido | segundo usuário corporativo **sem role prévia** |
| `oracle:read` concedido | contraprova com conta externa negada |
| Conexão real do Claude.ai | **smoke funcional: uma chamada real por tool** |
| Inventário das cinco tools | rollout para as demais organizações Claude |
| Observabilidade sanitizada em produção | — |
| Política corporativa do OM3 versionada | — |

**Gate OM2-C/OM2-D — o que foi provado, com escopo estrito (25/08).** A hipótese testada era que
o servidor **não implementava `initialize`/2025-06-18** — que só falaria a era 2026-07-28
(`initialize` respondendo `-32601`, `server/discover`, `Mcp-Method`/`Mcp-Name`/`_meta`
obrigatórios). **Essa hipótese foi falsificada.** `@modelcontextprotocol/server@2.0.0` — SDK
oficial — declara `SUPPORTED_PROTOCOL_VERSIONS = ["2025-11-25", "2025-06-18", "2025-03-26",
"2024-11-05", "2024-10-07"]`, e `createMcpHandler` usa `legacy: 'stateless'` **por padrão quando a
opção é omitida**, que é como `handler.ts` a invoca. Uma requisição **sem** o envelope `_meta` de
2026 — a forma claim-less do fluxo 2025-06-18 — é roteada para o leg 2025 e recebe `initialize` →
**200** com `protocolVersion: "2025-06-18"`, `capabilities.tools` e `serverInfo`;
`notifications/initialized` → **202**; `tools/list` → **5/5**; `tools/call` executa. **Zero chamada
ao backend em todo o handshake.** Comprovado também com o `Client` oficial pelo
`StreamableHTTPClientTransport` sobre HTTP real, em `versionNegotiation.mode: 'legacy'` explícito,
atravessando a fronteira OAuth com bearer sintético; a sequência emitida pelo cliente é **gravada e
asseverada**, não presumida.

**LIMITE DESTA PROVA.** Ela demonstra que o **servidor** cumpre o contrato quando um cliente
**oficial** o exercita. **Não é captura do Claude.ai:** nenhum tráfego do conector real foi
observado, e nada sobre o que o Claude.ai envia pode ser derivado daqui. Também **não** elimina
toda e qualquer falha de transporte específica do cliente real — só a hipótese nominal acima. Os
testes que "provavam" o contrário mandavam `initialize` **vestido de 2026** (com `_meta` +
`Mcp-Method`), forçando o leg moderno — eram corretos para o que testavam, mas seus comentários
generalizavam indevidamente, e foi essa generalização que virou a hipótese errada. **Nada do
protocolo foi alterado:** `handler.ts`, `server.ts`, `tools.ts`, `package.json` e
`package-lock.json` estão intocados; zero troca de SDK, zero adapter, zero tool nova. O gate
entregou **cobertura de regressão** e **correção documental**, nada mais.

**Como a causa foi identificada — registro do método, já resolvido.** No fechamento do OM2-D a
causa era desconhecida: sabia-se que o OAuth chegava a `Success Login` e `Success Exchange` e que o
servidor suportava o lifecycle 2025-06-18 localmente, mas **não** se sabia qual status o
`/api/mcp` devolvia na tentativa real — nem se chegava a ser chamado. A conclusão registrada
então foi que a evidência pendente era **uma só**: o status HTTP da primeira chamada após o
`Success Exchange`, porque cada status aponta para uma fronteira diferente (nenhuma chamada →
conector/callback; 401 → bearer; 403 → escopo; 404 → ambiente; 400/405 → transporte; 200/202 →
sequência MCP posterior; 429 → rate limit; 5xx → exceção). Optou-se por **instrumentar em vez de
adivinhar** — o OM2-E — e a decisão se pagou: os eventos mostraram **401 `bearer_missing`** no
discovery e depois **403 `insufficient_scope`**, isolando **autorização** como a fronteira, sem
tocar em protocolo, ambiente ou OAuth por hipótese. Token opaco/`aud` divergente, que era só uma
hipótese do ramo 401, **não** era a causa.

**Gate OM2-E — observabilidade sanitizada, versionada em `7c5aab5` e CONFIRMADA EM PRODUÇÃO
(25/08).** Foram os eventos dela que classificaram a fronteira da falha (`bearer_missing` →
`insufficient_scope`) e permitiram resolver a conexão sem alterar protocolo, ambiente ou OAuth por
hipótese. Como
o OM2-D ficou inconclusivo por falta do status real, o OM2-E torna a falha observável em vez de
adivinhar a causa. Cada requisição a `/api/mcp` passa a fazer **exatamente uma tentativa de
emissão** de um evento estruturado dizendo
em que fronteira terminou — `phase` (`environment`/`authentication`/`authorization`/`protocol`),
`outcome`, `http_status`, `http_method`, `jsonrpc_method` (allowlist fechada), `failure_category`,
`duration_ms` e um `correlation_id` gerado no servidor. **O schema é fechado em nove campos** —
`http_method` é vocabulário fechado (`GET`/`POST`/`DELETE`/`OTHER`, com qualquer outro verbo
colapsando em `OTHER`) e apenas `http_status` admite `null` — e não existe caminho de código que
copie para dentro dele token, segmento de JWT, tamanho do token, `kid`/`iss`/`aud`/`sub`/`azp`,
scope ou permissions recebidos, cookie, IP, user-agent, corpo, argumentos ou resposta de tool, ou
mensagem bruta da `jose`. A classificação do bearer usa só três sinais estruturais — header
presente, esquema `Bearer`, contagem de segmentos — o que separa `token_malformed_or_opaque` de
`jwt_verification_failed`, **a distinção que decide o ramo 401**.

Três precisões fecham o gate. **`jsonrpc_method` é sempre `unknown` em qualquer negação anterior à
autenticação**: o corpo não é lido no 404 de ambiente nem no 401/403, porque inspecionar payload
não autenticado só para enriquecer um log contradiria o próprio OAuth-first — depois do bearer
validado, e só então, o método volta a ser identificado, sob o mesmo teto de 64 KiB.
**`http_status: null` significa exatamente "nenhuma `Response` foi observada por esta camada"** —
o caminho terminou em exceção relançada e quem produz o status final é Next/Vercel; um 5xx
recebido como `Response` registra o status exato. E **só 2xx conta como sucesso**: 3xx passa a ser
`unexpected_redirect` (o endpoint nunca redireciona, então indica proxy ou reescrita) e 429 ganhou
categoria própria, `rate_limited`, em vez de `none`.

A resposta ao cliente é byte-equivalente: nenhum header novo, nenhum status alterado, e um logger
que lança não afeta a rota. **Nenhuma causa foi corrigida** — nada de Auth0, OAuth, protocolo,
variável ou dependência; apenas instrumentação. Suíte **1.110/1.110** e MCP **282/282** verdes;
typecheck e build limpos.

**Gate OM3 — acesso corporativo automático: POLÍTICA VERSIONADA; ACTION NÃO PUBLICADA NO AUTH0
(25/08).** Hoje o
acesso ainda depende de **role manual** por pessoa. O OM3 entrega uma **Post-Login Action (trigger
v3)** que concede `oracle:read` automaticamente a quem autentica pelo Google com e-mail
**verificado** em `gocase.com` ou `gobeaute.com.br`, vindo de um OAuth Client do Oráculo — negando
todo o resto com `api.access.deny()`. A auditoria confirmou o que viabiliza a decisão:
`effectiveScopes` **une** os claims `permissions` e `scope`, então `addScope("oracle:read")`
autoriza sem RBAC, sem credencial M2M, sem rate limit da Management API e **já no primeiro token**
— o que uma atribuição de role via Management API não faria. Comparação de domínio por
**igualdade**, nunca `endsWith`; cliente fora da allowlist sai sem `deny` e sem `addScope`, de modo
que nenhuma outra aplicação do tenant é afetada. **34 testes puros** cobrem os dois domínios,
normalização de caixa, vizinhos maliciosos (`evilgocase.com`, `gocase.com.evil.example`,
`sub.gocase.com`, `gocase.com.br`), Gmail pessoal, e-mail ausente/não verificado/malformado,
conexão não-Google, audience divergente, precedência do hosted domain e ausência de qualquer scope
de escrita.

**Um único handler cobre login inicial e refresh.** O Post-Login executa nos dois fluxos, que se
distinguem por `event.transaction.protocol === "oauth2-refresh-token"` — valor documentado pelo
Auth0. **Não existe `onExecuteCredentialsExchange`**, que é o trigger de Client Credentials/M2M e
não cobriria refresh. A política não ramifica por protocolo: um refresh não pode virar caminho para
escapar da regra do login inicial. O hosted domain vem de **`event.user.idp_tenant_domain`**, onde
o Auth0 mapeia o `hd` do Google; fallback coincidente não pode mascarar divergência da fonte
primária.

**Política versionada no commit de fechamento deste gate; NADA foi aplicado no Auth0.** Versionar
o código não o coloca em vigor: a Action segue **não publicada**, e por isso **o acesso continua
dependendo de role manual**. Não há ferramenta autenticada de Auth0 nesta sessão e nenhuma foi
instalada; o agente entregou código, testes e procedimento, e parou antes da mutação externa.
**Pendente do proprietário:** publicar a Action em trigger v3, configurar os secrets, arrastá-la
para o Login Flow, e então executar o smoke de identidade — proprietário, **um segundo colaborador
corporativo sem role manual prévia** e uma contraprova externa. Só depois disso o smoke funcional
das cinco tools. Rollout multi-organização, onboarding, offboarding e rotação em
[ORACLE_AUTH0_ACCESS_RUNBOOK.md](ORACLE_AUTH0_ACCESS_RUNBOOK.md). **Zero banco, zero tool nova,
zero endpoint novo, zero scope além de `oracle:read`, zero secret no repositório.**

**Contrato de entrega, sem exagero:** exatamente uma tentativa de emissão por requisição e **no
máximo um** evento persistido. Com logger funcional é exatamente um — é o que a suíte verifica,
com logger injetado. Como logging é best-effort e falha do logger não pode afetar a rota, uma
falha do provedor resulta em **zero** evento, nunca em erro; não há retry nem fallback.

**Versionado em `7c5aab5` e confirmado em produção.** A publicação saiu pelo **deployment
automático da Vercel** disparado pelo push — nenhum deploy manual foi executado. A tentativa
manual de conexão seguinte produziu os eventos que decidiram o ramo: **401 `bearer_missing`** no
discovery, depois **403 `insufficient_scope`**. **Nenhuma correção de OAuth, audience, issuer,
scope ou protocolo foi aplicada por hipótese** — a instrumentação apontou a fronteira, e a
correção foi feita no Auth0 pelo proprietário, com base na evidência.

> Redação anterior, preservada como histórico do fechamento do OM2-E: *"Nada aqui indica que o
> conector funcione; ele continua não conectado."* Superada — o conector conectou.

O bloqueio de identidade foi **resolvido**: o proprietário criou o tenant Auth0
`gobeaute-oraculo`, com **Google apenas como provedor de login**, audience
`https://mktplace-gobeaute.vercel.app/api/mcp`, permission `oracle:read`, role atribuída ao
usuário autorizado e callback `https://claude.ai/api/mcp/auth_callback` — **configuração de
dashboard informada pelo proprietário, não verificada pelo agente**. O `/api/mcp` passou a ser
um **OAuth 2.1 resource server**: valida JWT por JWKS do Auth0 (RS256, `iss`/`aud`/`exp`/`nbf`/
`sub`), expõe Protected Resource Metadata (RFC 9728) nas duas formas well-known, e responde
**401** sem credencial, **403** sem `oracle:read` e **404** quando a configuração está
incompleta. **Produção da Vercel só é elegível com todos os guardrails; Preview, custom
environments e host desconhecido respondem 404 sempre** — a classificação usa os sinais
explícitos da plataforma, **nunca `NODE_ENV`**, porque a Vercel executa Preview com
`NODE_ENV=production` (um fail-open real, encontrado e corrigido na revisão pré-versionamento).
A permission `oracle:read` é **constante no código**: configurar outra derruba a configuração
inteira. Nenhum Client ID/Secret entra no ambiente do MCP — a validação usa apenas issuer e JWKS
públicos. **Suíte completa 100% verde — 1.079 testes, 251 do MCP** (medido em 25/08 sobre
`76f361b`; as contagens `968`/`986` que constavam aqui eram anteriores ao rebase e estavam
erradas), typecheck e build limpos; uma dependência de runtime adicionada (`jose`, sem
vulnerabilidade atribuída, com mudança de **uma linha** no lockfile). O `npm audit --omit=dev`
reporta **4 vulnerabilidades high preexistentes** (`next`, `postcss`, `sharp`, `nanoid`), nenhuma
atribuída ao `jose`. **O que já foi publicado:** o código está em produção (`7b1b451` e `7c5aab5`),
o proprietário configurou as cinco variáveis em Production, redeployou e **conectou o Claude.ai**.
**O que NÃO foi executado pelo agente:** nenhum deploy, variável, segredo, alteração de Auth0 ou
conexão de conector — todas essas ações foram do proprietário. **Próximo passo: versionar o OM3 e
publicar a Post-Login Action v3**, para que o acesso deixe de depender de role manual; depois, o
smoke funcional das cinco tools. **O access token não deve ser copiado, colado ou impresso em
chat, terminal, documento, fixture ou log** — ele é armazenado pelo Claude.ai, o proprietário não
o controla, e o diagnóstico foi desenhado justamente para não depender de vê-lo.
Servidor MCP **read-only** implementado em `/api/mcp` (Next.js, runtime Node, stateless),
expondo cinco tools de leitura sobre a API pública do Render: desempenho dos marketplaces,
comparação de canais × marcas, produtos prioritários, qualidade/frescor dos dados e vendas
por UF. **Nenhuma tool de escrita, nenhum acesso direto a banco, nenhuma PII.** Integrado
sobre o **V3-1A (`e675948`)** com a suíte completa **100% verde** (875 testes, dos quais 168
do MCP), typecheck e build limpos. QA local real executado com o cliente MCP oficial por
Streamable HTTP contra o backend público: **52/52**, com reconciliação REST × MCP de
divergência zero — exceto a diferença **contratual e esperada** entre GMV regional e
gerencial, que é de cobertura e continua sem qualquer ajuste artificial.
**O que o OM2-P0 fechou:** a auditoria de compatibilidade, com fontes oficiais, concluiu que
o **custom connector do Claude.ai não permite configurar header estático** — aceita a URL do
servidor e, opcionalmente, credenciais **OAuth**. Portanto **bearer compartilhado não conecta
o Claude.ai**, e permanece no máximo como alternativa **local e opcional** para Claude Code,
**não aprovada**. A conexão do Claude.ai está **`BLOCKED_BY_IDENTITY_PROVIDER`**:
**OAuth não foi implementado** e **não existe issuer escolhido**.
**Estado da rota:** `/api/mcp` **permanece 404** em Production, Preview e custom environments,
por construção; **o Claude.ai não foi conectado** e nenhum conector externo foi configurado.
A Vercel **não** oferece exceção de Deployment Protection por rota, e o Automation Bypass
destravaria o projeto inteiro — nenhum dos dois conta como autenticação.
**Decisão pendente do proprietário: escolher o issuer/provedor de identidade** — sem ela não
há gate seguinte a executar. Detalhes em [ORACLE_MCP_PLAN.md](ORACLE_MCP_PLAN.md) §28 e §29.

**Gate V3 — Inteligência e Marca 360 (25/08/2026): `ENCERRADO COMO PASS. SMOKE TERMINAL APROVADO EM PRODUÇÃO DEPOIS DA REPUBLICAÇÃO MANUAL DO BACKEND`.**
O **PF1** saiu em `45fa3f8`: `withCache` não guarda mais resposta degradada, então
"Tentar novamente" voltou a consultar a rede. O **V3-BE** saiu em `26434c8` com
**BE3, BE4, BE5 e BE6**, e o smoke read-only de produção confirmou o contrato
servido — 75 verificações, zero falha. Com o `opportunity_map` publicado, o
**V3-1B** implementou a **matriz 2×2 definitiva** consumindo o contrato sem
recalcular mediana, referência de ROAS, classificação, agregados, faixas ou
seleção de destaques no frontend, e foi integrado a `7b1b451` preservando a frente
Oráculo/OAuth. O **QA visual final foi executado em navegador real** (Chromium do
cache local, build de produção, três viewports 1440×900/1024×768/390×844):
**96 verificações, 96 aprovadas**. A rodada terminal fechou dois achados de
aplicação — a dica de rolagem compartilhada renderizava **11px**, abaixo do piso de
12px do V3, e as duas referências de GMV apareciam **abreviadas e idênticas**
(`R$ 2K` para 2.207,05 e para 1.816,73), o que apagava justamente a mudança de
escopo que o diálogo afirma existir. **Dívida preexistente registrada e não
corrigida:** a mensagem de 422 do `resolve_brands` ecoa o valor inválido recebido —
é JSON, não executa HTML e não expõe stack, SQL nem DSN. O **V3-1B foi versionado em
`2d7ecdf`** (12 arquivos, `+2048 −96`). Em seguida o **V3-2** reconstruiu `/brand/[brand]`
como Marca 360: `available_months` do BE5 substituiu a lista de competências que vinha de
um módulo de **mock**, `ref_month=YYYY-MM` tornou a seleção mensal compartilhável, os dois
regimes temporais passaram a ter formatação e etiqueta distintas em **todo** bloco, o
contexto quente ganhou a origem `inteligencia` com `ctx_focus` próprio (união discriminada,
com Canais intocado), nasceram os dois drill-downs mensais no shell único, e o bloco de
**Demographics saiu** — as sete colunas de audiência são 100% nulas na fonte. **1131 testes,
1131 aprovados; typecheck limpo; build compilando. Nenhum commit, push ou deploy; nenhum
backend, endpoint, SQL, pipeline ou dependência.** Em seguida o **V3-3** executou o QA
visual integrado em navegador real, com a base já integrada até `76f361b`: **260
verificações em três viewports, 248 aprovadas**, em duas camadas declaradas —
backend real para o happy path e fixture determinística só para os estados que a
fonte não produz sob demanda. As 12 não-aprovações são **quatro dívidas
preexistentes de componentes compartilhados** (`DailyChart` com rótulo de eixo a
10px e `svg` sem nome, `MarketplaceFilter`/`DateRangeFilter` com alvos de 32–36px),
cada uma demonstrada também em `/`, `/canais` ou `/produtos`. **Um patch terminal
fechou as quatro** antes do versionamento, porque esses componentes aparecem
diretamente nas páginas validadas: ticks de 10px e 11px foram a 12px nos dois
gráficos da Marca, o gráfico ganhou `role="img"` com nome e descrição derivados do
dado, a legenda virou texto com marcador decorativo, e `MarketplaceFilter`,
`DateRangeFilter` e `PeriodSelector` foram a 44×44px. O *sticky* da Gerencial
cedeu **espaçamento**, nunca o alvo. Medido em **9 rotas × 3 viewports: 154
verificações, 154 aprovadas**, com zero overflow, zero recorte e zero erro de
console em 27 carregamentos. O veredito do V3-3 passou de `PASS WITH ISSUE` a
**`PASS`**. Uma **única rodada consolidada** fechou sete achados reais,
dos quais dois materiais: a competência mensal usava `router.replace` e o botão
voltar **pulava** a escolha do analista, e nove `toFixed` reintroduziam **ponto
decimal** numa interface pt-BR — o mesmo defeito que o V3-1A já havia fechado na
Inteligência. **1169 testes, 1169 aprovados; typecheck limpo; build compilando.
Nenhum commit, push ou deploy; nenhum backend, endpoint, SQL, pipeline, banco ou
dependência. O Gate V3 está tecnicamente fechado e aguarda revisão.**

Dois patches terminais fecharam as pendências do smoke: **`a893a2b`** uniformizou as
duas contagens do diálogo de quadrante com `contagemExata`, sem tocar o `fmtNumber`
global, e **`aa6e245`** preservou o foco nas transições internas da matriz. O
diagnóstico anterior de "dois diálogos" estava errado: o segundo `role="dialog"` era o
`MobileDrawer` da navegação, sempre montado e oculto. O `KpiDrilldownDialog` continua
sendo um só — o efeito de foco dependia apenas de `open`, e `dialog.kind` mudava de
`matrix` para `quadrant`, `band` ou `point` com `open` ainda `true`, desmontando o botão
acionador e deixando o foco no `document.body`. A prop opcional `focusResetKey` deu ao
foco um gatilho por conteúdo, sem recapturar `previousFocusRef` nem executar cleanup de
`inert`/`overflow`; consumidores que não passam a prop ficaram idênticos.

**O smoke terminal reprovou na primeira tentativa (`NO-GO`)** por um motivo só: o
`reference_note` chegava **sem acentuação** em produção. A causa foi medida — o texto vem
do payload, o frontend não tem fallback para ele, e `gold_service.py` já continha a
string acentuada desde `84c9d5f` —, logo o Render rodava uma revisão anterior. Nenhuma
linha de código precisava mudar e a documentação não foi alterada naquela rodada. **O
proprietário republicou manualmente o backend no Render** e o reteste focal, ainda
read-only e sem interceptação, aprovou: **um GET no endpoint** devolveu 200 com o
`reference_note` exato (`Referências descritivas do portfólio no escopo atual; não são
metas comerciais.`), `total_count = 1650`, `returned_count = 40` e quadrantes
`493/187/57/162` com 10 destaques cada; e a **confirmação visual focal em 1440×900 e
390×844 mediu 31 verificações, 31 aprovadas** — universo `1.650` no cabeçalho e na
declaração, `40` destaques, quadrante `493`/`10`, zero K/M nessas contagens, texto
acentuado nos dois viewports, nenhuma mensagem de fallback, zero erro de console e de
hidratação, zero overflow horizontal. A contraprova de foco no mobile (matriz →
quadrante) terminou em `Fechar detalhes`, **nunca** em `body`, com um único diálogo
visível, `inert` e `overflow` mantidos durante a transição, e Escape restaurando os dois
e devolvendo o foco ao acionador original. As outras cinco jornadas de foco não foram
repetidas: nada do frontend mudou desde a rodada anterior, em que passaram.

Nem a Vercel nem o Render expõem o SHA publicado, então a leitura honesta é
**comportamento do frontend compatível com `aa6e245`** e **backend servindo a nota
acentuada de `84c9d5f`** — não afirmação de SHA. **Dívidas que o `PASS` não cobre e que
continuam abertas:** as quatro ocorrências de `gray-on-color` do detector de design
(`KpiDrilldownDialog.tsx:142` e `inteligencia/page.tsx` 364/378/657) e o **422** do
`resolve_brands`, que ecoa o valor inválido recebido. Os estados `empty`, `unavailable` e
`error` não foram reverificados neste reteste — produção estava saudável e a rodada
proibia interceptação; o `loading` foi verificado de forma real. **UE3 foi iniciado
depois disso — ver a seção do Gate UE3 abaixo.**

**Gate V3-1A — patch terminal de acessibilidade (19/08/2026): `A1–A4 FECHADOS`.**
Segunda correção estreita, **explicitamente autorizada pelo proprietário** depois do
stop-loss do QA visual. O contrato autoritativo foi aplicado sem desconto — **todo
controle interativo com 44×44px renderizados e nenhum glifo abaixo de 12px** —, e os
24px, 30px e 40px deixaram de ser tratados como dívida aceitável. Medido em navegador
real com `getBoundingClientRect()`/`getComputedStyle()` nos três viewports
(1440×900, 1024×768, 390×844): navegação interna de **24px → 44×44px** (rótulos longos
mantêm a largura natural até 115,2px), segmentos do Pareto de **~30px → 44px**,
cabeçalhos ordenáveis de **~40px → 44px** e glifo de ordenação de **10px → 12px** nos
12 glifos. Duas descobertas só apareceram na medição: `h-11` com `border` rendia
**42px** no `border-box`, corrigido movendo a garantia para o próprio botão
(`self-stretch min-h-11`); e o rótulo curto "LTV" media **39,8px** de largura,
resolvido com `min-w-11` como piso, sem travar os rótulos longos. **Correção de
classificação:** o glifo de 10px não era do V3-1A — vive no `SortableHeader`
**compartilhado por 14 arquivos e 156 usos**, e é dívida anterior ao gate. Por isso a
regressão do componente compartilhado foi feita em duas frentes: **66 cabeçalhos**
medidos em `/canais`, `/produtos`, `/qualidade` e `/inteligencia`, em desktop e mobile,
todos ≥44px com glifo a 12px e ordenação funcional; e auditoria estática dos 156 usos,
que mostrou **nenhum token de altura** passado por consumidor algum — `className` vai
para o `<th>`, nunca para o `<button>`, então `min-h-11` não é sobrescrevível.
`/regioes` foi **aberta em desktop e mobile** e verificada quanto a viewport, layout e overflow — **não houve overflow horizontal** —, mas **a tabela não montou**, então **nenhum `SortableHeader` daquela rota foi medido em runtime**. Ali a garantia é **indireta**: o mesmo componente compartilhado com `min-h-11` e a auditoria estática que não achou override de altura em nenhum dos 156 usos. Fica como **risco não bloqueante**. Contratos funcionais confirmados na renderização: navegação
interna chegando às seções certas, ordenação invertendo valores de verdade com
`aria-sort` alternando, Pareto abrindo o bucket correto, diálogo com focus trap/Escape/
devolução de foco, foco visível por teclado, zero overflow horizontal e **zero erro de
aplicação ou de hidratação**. Validações: **707/707** testes (12 novos), typecheck
limpo, build compilando, `package-lock.json` intocado, zero dependência nova, nada fora
de `apps/web/` e `docs/`, detector com apenas os 3 falsos positivos já pré-classificados
e scan de secrets/PII sem ocorrência. Dívida preexistente registrada e **não**
corrigida: `/pedidos` rebaixa o rótulo do `<th>` a 10px via `!text-[10px]` em 7 usos.
**Dívida prioritária de plataforma:** `withCache` armazena respostas degradadas/falhas
por até cinco minutos, tornando "Tentar novamente" ineficaz em algumas telas; correção
deve ocorrer em gate próprio **PF1**, antes do V3-1B — **não foi causada pelo V3-1A**.
**Nenhum commit, push ou deploy; o worktree segue isolado em
`ui-v3-inteligencia-marca`.**

**Gate V3-1A — QA visual (18/08/2026): `EXECUTADO EM NAVEGADOR REAL — 1 CORREÇÃO
APLICADA, 4 ACHADOS ABERTOS`.** O QA rodou em Chromium 149 dirigido por Playwright
1.62.1, ambos já presentes no cache da máquina — **nenhuma dependência instalada** e
`package-lock.json` intocado. Como o backend real não estava alcançável, a matriz de
estados foi provada por **interceptação de rede determinística** sobre a build de
produção, com fixture declarado e números calculados à mão antes de olhar a tela;
nada foi fabricado para encobrir a ausência do backend. Provado por renderização:
a **fila preserva cada linha exatamente uma vez** (6 linhas para 6 de fonte, duas
duplicatas de `(marca, título)` e uma colisão entre listas sobrevivendo, somas
R$ 700 / R$ 4K / R$ 800 conferidas), **`null` continua distinto de zero** (`0,0%`
para zero real, `—` para nulo), `lens` reproduzível por URL sem propagação pela
sidebar, a rota canônica `/produtos?channels=ml&brands=…&pareto_bucket=…` chegando ao
endpoint como `brand` singular, todos os drill-downs com foco preso e devolvido, e
**zero erro de console, zero request de escrita e nenhum host além do local** em 849
requests. **Uma correção consolidada** foi aplicada: a tela renderizava todo
percentual, ROAS e nota com **ponto decimal** (`12.0x`, `0.0%`) porque vinham de
`toFixed`, insensível a locale, enquanto o dinheiro na mesma tela já usava pt-BR —
corrigido em 14 pontos com um módulo de formatação novo, **somente apresentação**,
sem tocar cálculo, métrica ou threshold. Ficaram **4 achados abertos** de alvo e
tipografia (A1–A4 no §20.9 do plano; todos passam o mínimo AA de 24×24px, o mais
material é a navegação interna com alvo de 24px no mobile) e **1 achado de plataforma
fora do escopo**: `withCache` memoiza a falha da API por 5 minutos, o que deixa o
"Tentar novamente" inerte nesse intervalo — defeito **anterior ao V3-1A**, em
`api-client.ts`, que afeta todas as telas. Validações após a correção: **695/695**
testes, typecheck limpo, build compilando, detector com apenas os 3 falsos positivos
já pré-classificados e scan de segredos/PII sem ocorrência. **Nenhum commit, push ou
deploy foi feito; o worktree segue isolado em `ui-v3-inteligencia-marca`.**
...
|---|---|---|---|---|
| Gate V3 — Inteligência e Marca 360 | **V3-0, V3-1A, PF1, V3-BE e V3-1B VERSIONADOS (`309b6bf`, `e675948`, `45fa3f8`, `26434c8`, `2d7ecdf`); V3-2 (Marca 360) e V3-3 (QA integrado `PASS`, 402/414 em três viewports, patch de a11y compartilhada fechado) TECNICAMENTE CONCLUÍDOS E VERSIONADOS no commit de fechamento do Gate V3 — PUBLICAÇÃO E SMOKE PÓS-DEPLOY PENDENTES** | Desenho de 18/08/2026: auditoria ponta a ponta da referência `torre_b2b` (clone temporário, removido) e das duas superfícies da Torre, verdade dos dados dos 7 blocos de `/inteligencia` e dos 5 de `/brand-detail`, matriz de drill-down, wireframes desktop/mobile e 13 estados. Achados materiais: Inteligência **não tem nenhum drill-down** e tem falsa affordance em 3 tabelas; `ML_BRANDS` no frontend omite **rituária**; contagens exibidas são as **capadas** (LIMIT 30/20/20); **seis dos sete blocos não têm período** porque a fonte é fotografia sem dimensão temporal; Marca tem **dois regimes temporais sem marcação** e a lista de competências vem de um módulo de **mock**; a seção Demographics é 100% NULL na fonte. Correção terminal em 18/08 reescreveu sete contratos: matriz dependente de **BE6 `opportunity_map`**; drill-down do Pareto restrito aos agregados que o payload realmente traz (o payload **não** contém os produtos do bucket); contexto `ctx_*` **discriminado por origem**, com `ctx_focus` próprio da Inteligência em vez de reutilizar o `ctx_signal` de Canais; **BE5 `available_months` como bloqueio do V3-2**; renomeação do bloco de prioridades para deixar explícito que é fotografia ML sem janela; remoção do benchmark por mediana entre as três superfícies TikTok, que não são pares comparáveis; e o estado do próprio gate. V3-1A entregue em 18/08: `/inteligencia` reescrita em sete blocos (radar em vez de sete tabelas), marcas derivadas do payload — **rituária deixou de ser invisível** —, falsas affordances removidas, fila única com lentes reproduzíveis pela URL, LTV filtrável, e o wiring canônico `/produtos?channels=ml&brands=…&pareto_bucket=…`. A matriz 2×2 **não** foi implementada: degradou para faixas de amostra priorizada, porque depende de **BE6**. Uma rodada de correção consolidada pré-QA fechou cinco findings, dois deles de comportamento: **`buildQueue` apagava linhas reais** — deduplicava por `(brand, title)` quando o grão da fonte é `(brand, item_id)` e o payload não traz `item_id`, o que fazia duas linhas com o mesmo título e ad spend 100 e 200 virarem **uma linha e uma soma de R$ 100 em vez de 300** —, e **o carregamento era apresentado como vazio**, com a página dizendo "Nenhuma prioridade" e "Sem dados" antes de a requisição terminar. Também fecharam o drill-down e as CTAs do bloco de produtos, a redação de truncamento ("5 de ao menos 30", nunca "de 30" como total) e o cabeçalho da coluna de ação do LTV. Nenhum backend, endpoint, dependência ou pipeline tocado. O **QA visual foi executado em navegador real** (Playwright + Chromium do cache local, três viewports) e o resultado é **`PASS` nos contratos executados**: fila preservando cada linha uma única vez, `null` distinto de zero, `lens` reproduzível por URL, rota canônica para `/produtos`, drill-downs com foco preso e devolvido, zero erro de console e de hidratação. Duas correções saíram dele — separador decimal pt-BR e o patch de acessibilidade que **fechou A1–A4** (alvos de 24/30/40px para **44×44px** e glifo de 10px para **12px**, medidos com `getBoundingClientRect()`/`getComputedStyle()`). Validações finais: **707/707 testes, typecheck limpo e build verde**. | **1)** ~~PF1~~ **versionado em `45fa3f8`**; **2)** ~~contratos backend BE3/BE4/BE5/BE6~~ **versionados no V3-BE (`26434c8`), contrato confirmado em produção**; **3)** ~~V3-1B~~ **matriz definitiva versionada em `2d7ecdf`, QA visual final 96/96**; **4)** ~~V3-2~~ **Marca 360 implementada, aguardando revisão**; **5)** ~~V3-3~~ **QA integrado executado em navegador real, 248/260**. Nenhuma fase do Gate V3 segue aberta tecnicamente. | **BE6 e BE5 foram entregues no V3-BE** — a matriz do V3-1B e o V3-2 deixaram de estar bloqueados; **BE1 e BE2 foram retiradas** — o `item_id` passou para dentro do contrato BE6, e acrescentar `product_id` a `tk_products` mudaria `GROUP BY`/grão/valores, virando decisão futura separada; BE3 e BE4 seguem aditivas e não travam a fundação. GMV TikTok com frete continua **decisão aberta** e não foi incorporado. |
...
pós-publicação `PASS WITH ISSUE`.**

Backend publicado **manualmente pelo proprietário** no Render; frontend pelo
**fluxo automático da Vercel**. O smoke read-only confirmou **comportamento
compatível com `8760f96`**: o `openapi.json` de produção publica os contratos que
só existem nesse commit — `affiliate_costs` em `CanaisResponse`, os três modelos
e as quatro dimensões ortogonais com os enums exatos. **Cinco cenários de API** e
**navegador real em 1440×900 e 390×844** passaram; os **15 valores** de julho/2026
são idênticos entre JSON e tela, sem recálculo, inversão de sinal, `abs()` ou
soma. **Latência total de produção: mediana de 423 ms** no cenário com o bloco
(p95 810 ms, ~17 KB, zero erro em 10 chamadas) — mas os cenários têm workloads
diferentes, então **essa diferença não é atribuível ao bloco** e o **RTT
Render→Neon segue não medido**.

A ressalva do `WITH ISSUE`: um `404` de console apareceu **uma vez** no desktop,
sem URL capturada, e **não reproduziu em seis tentativas** posteriores — nenhuma
delas de `/canais` ou do bloco, que renderizou corretamente em todas. Observação
intermitente, não atribuível a esta frente e não bloqueante.

Contexto anterior — **estado no checkpoint imediatamente após o versionamento,
superado pelo smoke da §24.11.**

Implementação, testes e QA integrada concluídos e versionados em um único commit
com os 14 arquivos da frente. *(À época)* o bloco ainda não podia ser considerado
disponível em produção: o backend dependia de publicação manual no Render e o
frontend, da publicação automática da Vercel, e o smoke ainda estava pendente.
**→ Hoje: ambos publicados e o smoke executado como `PASS WITH ISSUE`.**

A **QA integrada** rodou com backend local lendo o **Neon real** (somente
`SELECT`/`EXPLAIN`) e frontend servido pelo build de produção. **Oito cenários
read-only** contra dados reais e **QA visual em navegador nos três viewports
(1440×900, 1024×768, 390×844), com zero falha e zero aviso**. Uma única correção
foi necessária: o skeleton do bloco renderizava uma região `aria-busy` **sem
texto**, então um leitor de tela anunciava "ocupado" sem dizer do quê — todas as
outras regiões de carregamento de `/canais` mantêm o heading. O título passou a
aparecer no skeleton, com a animação restrita às barras de placeholder. Nenhuma
regra de contrato, fonte ou métrica foi tocada. Um segundo candidato foi
**descartado com prova**: a requisição observada ao abrir o diálogo é prefetch
RSC do Next para os `<Link>` das tabelas de marca — o painel não tem âncora
alguma, e rolar a página sem abrir o diálogo dispara os mesmos prefetches.

**Latência decomposta e sem extrapolação:** 0,202 ms de execução no Postgres,
232,13 ms de RTT desta máquina, e o endpoint `/canais` medindo 1174,90 ms com o
bloco contra 944,12 ms sem ele — **delta de +230,79 ms, praticamente um RTT
exato**. O bloco custa **uma ida-e-volta**, verificado por espião. O RTT é
**desta máquina** e **não é extrapolável** para Render→Neon. A latência total em
produção foi medida depois (§24.11); o RTT interno segue não isolado.

**Evidência de ausência vs. zero, medida:** 15 componentes com valor exatamente
`0` (zero medido, exibido `R$ 0,00`) e nenhum `null` nos payloads reais. A
ausência que ocorre nos dados é de outra natureza e foi verificada — competência
inteira ausente (2025-01) e marca ausente dentro da competência (2025-06 com 1/5,
2025-07 com 4/5) —, sempre expressa como **linha inexistente**, nunca como zero.

Contexto anterior — **Task 2/3 implementada e testada localmente.**

A **Task 1/3** desenhou e versionou o contrato de exposição (§23 de
[UNIT_ECONOMICS_SOURCE_CONTRACTS.md](UNIT_ECONOMICS_SOURCE_CONTRACTS.md), commit
`5bad294`). A **Task 2/3** implementou esse contrato: o bloco **"Impacto de afiliados no
resultado"** passou a existir em `/canais`, alimentado pela fact mensal
`marts.fact_tiktok_affiliate_cost_order_monthly` já materializada no UE2-B. Registro
factual da implementação na **§24** do mesmo documento.

**O que foi feito, de forma aditiva.** Backend: três modelos e cinco aliases `Literal`
novos no schema, um serviço dedicado e a composição do bloco **na rota** `/canais` —
não dentro de `get_canais`, que segue emitindo exatamente uma consulta e não conhece o
bloco. Isso isola o **contrato histórico** e a **falha esperada**, **não** a latência: a
composição é síncrona e `/canais` ganhou trabalho adicional — uma consulta, medida no
preflight read-only registrado na §24.6. Frontend: tipos no cliente, um módulo puro de apresentação, um painel que reusa o
`KpiDrilldownDialog` existente e a integração na página sob a mesma guarda de frescor dos
outros estados. **Zero endpoint novo, zero migration, zero dependência nova, zero
arquivo em `pipelines/`, `db/` ou `alembic`, `package-lock.json` sem diff.**

**Validação local:** 40 testes focais de backend e 33 de frontend, todos passando; suíte
`apps/api` em 720 passed contra 680 na baseline do `HEAD` limpo, com **as mesmas 43
falhas pré-existentes** nos dois lados — regressão zero. `npm test` 1290/1290,
`typecheck` e `build` limpos, `git diff --check` limpo, scan de secrets/PII sem achados.
As 43 falhas pré-existentes são de ambiente: testes de router sem banco esperam `503`, e
`psycopg2` estoura `UnicodeDecodeError` ao decodificar a mensagem de erro de conexão em
cp1252 no Windows pt-BR.

**O que a implementação deliberadamente NÃO faz.** Não soma os três lançamentos
(criador, parceiro, Ads de afiliado) em nenhum nível — a decisão de qual subconjunto
constitui "custo de afiliado" está aberta e a ausência de sobreposição entre eles **não
está provada**. Não calcula total, razão sobre GMV, ROI, ROAS nem receita atribuída. Não
aplica `abs()`: exibe o sinal da fonte. Não converte ausência em zero nem zero em
ausência. Não rateia competência mensal para dias. **Não mostra número em período
parcial ou desalinhado ao mês** — um valor parcial pareceria comparável a um mês
fechado. ML e Shopee aparecem como **"Dados indisponíveis"**, nunca "Não aplicável":
afiliado existe nesses canais, o que falta é fonte confirmada.

**Rodada de correção pré-QA (27/08).** Dez achados corrigidos: a afirmação de
isolamento de latência foi retificada (a composição é síncrona), as duas consultas
viraram uma, competência inteiramente ausente deixou de sumir da análise, o relógio
passou a ser `America/Sao_Paulo`, o log da falha esperada perdeu o traceback, o
erro terminal deixou de virar skeleton eterno, o diálogo passa a fechar na troca de
filtro, e o timestamp com offset passa a ser convertido para BRT. **Preflight
read-only real contra o Neon**: a consulta executa em **0,2 ms** no servidor sobre
70 linhas, e **99,2%** dos ~233 ms medidos são RTT de rede desta máquina. **Plano e
execução no Postgres não apresentam risco material no volume atual**; o bloco
adiciona **uma consulta e um round-trip síncrono**, medido localmente em cerca de
**230,79 ms**. A latência **total** do endpoint em produção foi verificada depois
(mediana de 423 ms, §24.11); o que **continua não isolado** é o **RTT interno
Render→Neon**, que exigiria instrumentação interna do serviço. Detalhes nas §24.7,
§24.8 e §24.11.

**O que permanece aberto após a Task 3/3.** A **convenção de sinal contábil segue não
confirmada** (§23.14-A): a UI exibe o sinal da fonte sem afirmar o que ele significa. A
**frente UE2-C** — rotina e SLA de atualização da fact, hoje carga manual — **continua
não iniciada**, e por isso o frescor exibido é `manual_snapshot`, nunca "atualizado
agora". O **retorno de afiliados segue indisponível**: não há receita atribuída no grão
necessário. A **latência Render→Neon não foi medida** e só poderá ser após publicação.
No navegador, o estado terminal exercitado ao derrubar a rota é o ramo "bloco ausente" —
`apiFetch` degrada para mock em vez de rejeitar —, não a fase `unavailable`; ambos foram
...
**Estado em 02/09/2026: `GATE ENCERRADO — PASS WITH ISSUE`. A tela está
utilizável em produção.**

Frente nova, aberta e encerrada em 02/09/2026. Compara o **preço anunciado** das
lojas próprias no Mercado Livre com o **preço sugerido de revenda (PDV)** das
tabelas B2B. Detalhes, contratos, números e o registro completo do QA em
[monitoramento_precos_proprios.md](monitoramento_precos_proprios.md).

| Camada | Estado |
|---|---|
| Código versionado | ✅ `d772fe8` em `main` |
| Migrations 013/014 no Neon | ✅ aplicadas, `alembic_version = 014` |
| Dado publicado | ✅ 25.559 linhas ML + 221 de referência B2B |
| **Backend no Render** | ✅ **publicado manualmente pelo proprietário** |
| **Frontend na Vercel** | ✅ **publicado** pelo fluxo automático do push |
| API em produção | ✅ **HTTP 200**; contrato idêntico ao versionado |
| Reconciliação API × UI × Neon | ✅ fecha nos três lados: 855 = 855 = 855 |
| QA em navegador real (3 viewports) | ✅ **127 de 128** verificações |
| Smoke de produção | ⚠️ **PASS WITH ISSUE** — `favicon.ico` 404, pré-existente |

**Reconciliação nos três lados**, D−1 = 2026-09-01: monitorados 855, comparáveis
138, abaixo 18, na/acima 120, sem referência 523, ambíguos 0, stale 0, inativos
194. A soma dos seis status fecha em 855, igual à contagem no Neon e igual ao que
a UI exibe. Por marca: 309/219/145/182 idênticos em API e banco. A baseline
anterior foi reproduzida integralmente — o dado não mudou entre as rodadas.

**QA real em Chromium** nos viewports 1440×900, 1024×768 e 390×844, contra o
payload de produção e **sem nenhuma fixture**: KPIs iguais à API, aviso
observacional com as 6 ressalvas, zero termo de sanção, paginação cobrindo as 855
sem duplicidade, filtro de situação preservando o denominador, ausência impressa
como travessão e nunca como zero, zero notação compacta, 513 alvos todos ≥44×44,
6.565 nós de texto todos ≥12px, focus trap/Escape/devolução de foco no
drill-down, zero hydration warning e zero requisição 4xx/5xx inesperada.

**Um defeito real foi encontrado pelo navegador e corrigido.** A página inteira
rolava na horizontal nos três viewports (`scrollWidth` 1766 contra 1440;
`scrollTo(9999,0)` deslocava 326px de fato). Causa: as 12 colunas em
`whitespace-nowrap` dão à tabela min-content de ~1587px, e o overflow escapava do
`overflow-x-auto` do `TableScrollHint`. Corrigido em `d772fe8` com
`overflow-hidden` no wrapper da tabela — o padrão que `/pedidos` já usava —, com
teste de regressão, e reverificado depois do redeploy: **excesso 0px nos três
viewports**. As outras rotas da Torre já mediam 0px, o que descartou dívida do
shell.

**A ressalva do `PASS WITH ISSUE`:** `GET /favicon.ico` responde **404** e gera
um erro de console no primeiro carregamento. **Não é defeito desta frente** — não
existe favicon versionado no projeto e a resposta é 404 em qualquer rota da
Torre. Corrigir alteraria todas as rotas e ficou fora deste escopo.

**O que a frente deliberadamente não faz.** É observacional: não automatiza
preço, não emite alerta ou notificação, não fiscaliza revendedor, não aplica
sanção e não tem workflow jurídico. A comparação usa **PDV**, que **não é PMA**;
a referência não tem vigência declarada na origem, então **não existe comparação
histórica**. Cobertura é `advertised_only`: só o preço anunciado, sem frete,
cupom, subsídio ou checkout — e como o checkout se compõe de produto + frete −
cupom, **a direção do desvio é indeterminada**, o que a tela declara em texto.

**Limitações que seguem abertas.** Somente Mercado Livre — Shopee e TikTok não
têm catálogo confiável de preço anunciado e Amazon não tem fonte na Torre.
Lescent é monitorada e não tem tabela B2B. As **3 linhas ambíguas da Rituária**
...
origem. Os estados `stale_observation` e `non_comparable_reference_ambiguous` têm
0 linhas hoje e **não foram exercitados em tela**, só em teste unitário. O sync é
**100% manual**: nenhuma entrada no Scheduler, nenhum step no `full_daily`.
...
- Monitoramento de preços próprios **encerrado como PASS WITH ISSUE** em `d772fe8` (02/09/2026): backend publicado manualmente no Render, frontend na Vercel, API 200, reconciliação API × UI × Neon fechando em 855, QA real em três viewports (127/128) e um defeito de rolagem lateral encontrado pelo navegador e corrigido; ressalva única é o `favicon.ico` 404, pré-existente da Torre.
...
1. **Gate S3 Task 3/3 (operacao controlada), sob autorizacao separada.** A ordem e'
   obrigatoria e esta detalhada em [SERVING_AIRFLOW_PLAN.md §38.12](SERVING_AIRFLOW_PLAN.md):
   incorporar sem force o hotfix `a1c5ffe` (ja publicado) -> reconciliar a base do S3 -> aplicar 009/010/011 -> validar
   schema -> backfills autorizados -> reconciliar fonte x Marts -> **contraprova de
   `/produtos` ML antes e depois** (o sync deixou de ser upsert) -> so' entao
   versionar/enviar o codigo -> fast-forward do checkout operacional antes das 06:00
   -> smoke e observacao de uma execucao agendada. Inverter a ordem quebra producao:
   push antecipado faz o Render consultar tabelas inexistentes, e atualizar o
   operacional antes das migrations faz o `full_daily` falhar no preflight.
2. **Revisar o horario das 06:00**, a divida de maior impacto: e' quando a VPN tende a
   estar fora, e a tarefa roda com `LogonType=Interactive` — so' com o usuario logado.
3. **Decidir a cadencia do backfill historico periodico.** O lookback de 90 dias nao
   garante convergencia do historico, e `/brand-detail` aceita qualquer mes.
4. **Decidir o que `/tempo-real` deve significar.** A fonte e' intraday e horaria; sob
   o contrato D-1 a tela mostraria as horas de ontem rotuladas como hoje.
5. **CONCLUIDO (18/08): a execucao agendada do `full_daily` com os tres steps de
   serving foi observada** — dez steps `SUCCESS`, `LastTaskResult=0`, `STATUS GERAL:
   OK`, sem retry, e a fonte Creator em D-2 tratada honestamente. Ver
   [SERVING_AIRFLOW_PLAN.md §37](SERVING_AIRFLOW_PLAN.md). Fica pendente observar a
   primeira execucao agendada com os **doze** steps, depois da Task 3/3 do S3.
6. **Revisar o horario das 06:00**, a divida de maior impacto: e' quando a VPN tende a estar
...
7. **Decidir a cadencia do backfill historico periodico.** O lookback de 90 dias nao cobre
   correcao anterior a essa janela.
8. **Observar o comportamento com fonte atrasada.** O piloto rodou com os tres targets em
   D-1, entao o caminho `min(D-1, source_max)` com fonte em D-2 — a razao de existir da ponte
   — segue provado apenas por teste, nunca por execucao real.
9. **Somente depois: Gate S3** — migrar `/inteligencia`, `/tempo-real` e `/brand-detail`
   para o Neon, removendo a dependencia operacional dessas rotas do Data Mart.
10. **Obter nome/URL e o modelo de hospedagem do Airflow** (auto-hospedado ou gerenciado) e o acesso de leitura ao repositório — é a próxima ação do Gate S0. Depois disso, **provar de dentro do worker real** o acesso ao Data Mart e ao Neon. O **piloto técnico** do módulo de sync pode avançar antes disso, de máquina com VPN, mas **não** prova o Airflow. **S1 executado como `PARTIAL — PILOTO VALIDADO`; S2 FECHADO como `PASS COM RESTRIÇÃO` — ver a seção de fechamento acima.**
11. Integrar essa materialização ao **Airflow da organização** — que **existe** segundo o proprietário, mas **não foi localizado nem está visível** com as credenciais desta sessão, e cuja plataforma, URL, hospedagem e rede ainda não foram informadas; **nenhuma DAG foi inspecionada, configurada ou executada** —, eliminando a dependência síncrona Render → Data Mart/VPN.
12. Depois disso, tratar as dívidas menores de acessibilidade dos filtros (nome acessível e tamanho de alvo).
13. Manter a decisão do **GMV TikTok com frete** como frente separada.
14. Observar as próximas execuções diárias do `full_daily` agendado antes de considerar o horário 06:00 definitivamente estável.
15. Transferir a rotina manual Shopee e iniciar a configuração administrativa da API oficial.
16. Fazer discovery do Octaprice em paralelo, sem iniciar implementação prematura.
