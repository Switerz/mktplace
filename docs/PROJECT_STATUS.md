...
1. **Observar UMA execucao agendada do `full_daily`** (18/08, 06:00) com os tres steps de
   serving, **sem retry**, e comparar com o piloto manual de 17/08. E' o unico caminho ainda
   nao exercitado: o piloto validou a TaskKey manual, nao a automatica com dependencia de
   `daily_ml`/`daily_tiktok`.
...
4. **Observar o comportamento com fonte atrasada.** O piloto rodou com os tres targets em
...
5. **Somente depois: Gate S3** — migrar `/inteligencia`, `/tempo-real` e `/brand-detail`
   para o Neon, removendo a dependencia operacional dessas rotas do Data Mart.
6. **Obter nome/URL e o modelo de hospedagem do Airflow** (auto-hospedado ou gerenciado) e o acesso de leitura ao repositório — é a próxima ação do Gate S0. Depois disso, **provar de dentro do worker real** o acesso ao Data Mart e ao Neon. O **piloto técnico** do módulo de sync pode avançar antes disso, de máquina com VPN, mas **não** prova o Airflow. **S1 executado como `PARTIAL — PILOTO VALIDADO`; S2 FECHADO como `PASS COM RESTRIÇÃO` — ver a seção de fechamento acima.**
7. Integrar essa materialização ao **Airflow da organização** — que **existe** segundo o proprietário, mas **não foi localizado nem está visível** com as credenciais desta sessão, e cuja plataforma, URL, hospedagem e rede ainda não foram informadas; **nenhuma DAG foi inspecionada, configurada ou executada** —, eliminando a dependência síncrona Render → Data Mart/VPN.
8. Depois disso, tratar as dívidas menores de acessibilidade dos filtros (nome acessível e tamanho de alvo).
9. Manter a decisão do **GMV TikTok com frete** como frente separada.
10. Observar as próximas execuções diárias do `full_daily` agendado antes de considerar o horário 06:00 definitivamente estável.
11. Transferir a rotina manual Shopee e iniciar a configuração administrativa da API oficial.
12. Fazer discovery do Octaprice em paralelo, sem iniciar implementação prematura.
