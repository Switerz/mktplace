...
| Revamp UI/UX (Torre) | **PLANEJADO — PRONTO PARA U1** | Gate U0 (auditoria e especificação, sem implementação) concluído em 24/07, com roadmap U1–U6 corrigido e alinhado às decisões de produto já aprovadas — ver [UI_REVAMP_PLAN.md](UI_REVAMP_PLAN.md). | Gate U1 — fundação visual e shell (sidebar clara/lavanda + drawer mobile, atualização estreita do `DESIGN.md`, rede mínima de typecheck/build). | Nenhuma dependência bloqueante; aguarda aprovação explícita para iniciar implementação. |
