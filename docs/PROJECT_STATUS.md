...
**Gate OM1 — conector MCP do Oráculo (19/08/2026): `CONCLUÍDO LOCALMENTE, NÃO PUBLICADO`.**
...
**Ainda não publicado:** `/api/mcp` **não está habilitado na Vercel** (responde 404 por
construção em Production, Preview e custom environments) e **o site da Vercel ainda não está
conectado externamente ao Oráculo** — nenhum conector do Claude.ai foi configurado.
**O OM2 depende da decisão e da implementação de autenticação real.** Próximo passo da
frente: **decidir entre bearer de piloto e OAuth**, sem tratar bypass da Vercel como
autenticação — a plataforma não oferece exceção de proteção por path para GET/POST, e o
Automation Bypass destravaria o projeto inteiro. Detalhes em
[ORACLE_MCP_PLAN.md](ORACLE_MCP_PLAN.md) §28.
