...
| Revamp UI/UX (Torre) | **U1 CONCLUÍDO — PRONTO PARA U2** | Gate U0 concluído em 24/07 ([UI_REVAMP_PLAN.md](UI_REVAMP_PLAN.md)). Gate U1 (fundação visual e novo shell) concluído e aprovado em 24/07: sidebar clara/lavanda persistente no desktop, drawer mobile (com focus trap) e topbar compartilhados substituem o antigo `AppNav` por página; filtros globais, contratos de API e regras de negócio preservados; typecheck/build/testes passando. | Gate U2 — Gerencial completa e padrão de drill-down. | Nenhuma dependência bloqueante. |
