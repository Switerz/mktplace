...
**Próximo gate proposto**: Gate S4.3e (não autorizado ainda) — aplicar `ALTER FUNCTION public.awsdms_intercept_ddl() SECURITY DEFINER` mediante autorização explícita separada, depois repetir uma única execução real de `--validate-shopee-window-write-path` na mesma janela de controle para confirmar que o `CREATE TEMP TABLE` deixa de ser bloqueado. (Nota: esta numeração mudou — o que era "S4.3d" aqui virou o Gate S4.3d abaixo, que fortalece o *preflight* para detectar esse cenário antes de qualquer ação real; a correção em si — `ALTER FUNCTION` — passa a ser o Gate S4.3e.)
...
**Status**: implementado e testado, **zero conexão com banco real** nesta rodada. A correção de fato (`ALTER FUNCTION ... SECURITY DEFINER`) continua não executada — vira o Gate S4.3e, com autorização explícita separada.
