...
**Correção permanente no loader recorrente (2026-08-05)**: até aqui, a consolidação existia **apenas** no script corretivo de dados (`fix_shopee_product_dates.py`, uma execução única sobre jan–mai); o loader recorrente (`etl/load_shopee_products.py`) **nunca** a incorporou e continuava aplicando last-wins a cada nova carga. A tentativa de carregar jun/jul reencontrou o bug: 66 chaves colidiam (71 linhas excedentes) nos dois meses. A lacuna foi fechada de forma permanente incorporando a **mesma semântica aprovada** ao loader, como a função pura `_collapse_variation_collisions(aggregated)`, executada na Fase A (`_prepare_all_brands`) logo após `_aggregate` e antes de qualquer conexão — soma `gmv`/`units_sold`/`completed_orders`/`canceled_orders`/`unique_buyers`, recalcula `cancel_rate_pct`/`avg_price` dos totais e combina `variation_name` de forma determinística (`"; ".join(dict.fromkeys(...))`). Uma guarda defensiva (`_assert_unique_keys`) recusa abrir conexão na Fase B se qualquer marca ainda colidir na chave real. Contraprova em memória sobre jun/jul (sem banco): 1.043 → 972 linhas, **zero** duplicidade final, e totais mensais **idênticos** antes/depois (GMV jun R$ 6.521.807,04; jul R$ 5.512.907,44). Testes: `apps/api/etl/tests/test_load_shopee_products_collapse.py`. A chave única permanece a de quatro campos — `variation_name` **não** foi adicionado à chave.

**Execução real (2026-08-05, commit `21980a8`)**: a carga de junho/julho foi executada com o loader corrigido sobre o root isolado (59 arquivos Order reconciliados, hashes conferidos) — 972 linhas upserted no PG local (total 3.443) e sincronizadas ao Neon via `sync_produtos --source shopee --full` (`audit.source_sync_run=success`, 3.443/3.443). Local×Neon bytewise idênticos nas colunas de negócio; jan–mai preservado (2.471 linhas inalteradas); monitor Bug 8 aprovado (invariantes + 10/10 combinações marca×mês reconciliadas contra a fonte isolada, 39 grupos só-cancelados preservados); API de produção validada em jun e jul (summary batendo o Neon nas 10 combinações).
