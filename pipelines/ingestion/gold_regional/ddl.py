"""
Parsing e execução do DDL versionado
(`db/sql/gold/marketplace_region_daily_ddl.sql`) para o Gate 6A.

O arquivo .sql é a fonte de verdade para revisão humana (inclui comentários
com as decisões de design — Shopee dedup, fonte ML raw, Barbours,
timezone). Este módulo extrai só os statements executáveis e os roda dentro
de UMA transação Python explícita — controle de transação fica inteiramente
no código, não no texto do arquivo.

`marketplace_region_daily_ddl.sql` (operacional, sem "IF NOT EXISTS") é
usado aqui — não `marketplace_region_daily_draft.sql` (histórico de
decisões, mantido como registro mas não mais executado). Ver o cabeçalho
daquele arquivo operacional para a justificativa de não usar
"IF NOT EXISTS" numa primeira aplicação.

`parse_ddl_statements` é uma cópia deliberada (não import) da função
equivalente em `pipelines/ingestion/shopee_raw/ddl.py` — mesma lógica pura
de parsing, mantida independente para nunca criar uma dependência cruzada
entre os dois pacotes de ingestão (cada um deve poder ser lido/auditado
isoladamente).
"""
...
DEFAULT_DDL_PATH = Path(__file__).resolve().parents[3] / "db" / "sql" / "gold" / "marketplace_region_daily_ddl.sql"
...
def parse_ddl_statements(sql_text: str) -> list[str]:
    ...
    `COMMENT ON ... IS '...'` não é um fim de statement. Descarta
    BEGIN/COMMIT/ROLLBACK (controlados pelo chamador) e fragmentos vazios."""
    lines_no_comments = []
    in_string = False
...
def execute_ddl(write_url: str, ddl_path: Path = DEFAULT_DDL_PATH) -> list[str]:
    ...
    ROLLBACK integral — nada fica parcialmente criado. Sem retry automático.
    ...
    try:
        if not try_acquire_advisory_lock(conn):
            raise WritePreflightBlocked(
                ...
                "aplicação da Gold regional pode estar em andamento. Abortando sem tentar novamente."
            )
        try:
