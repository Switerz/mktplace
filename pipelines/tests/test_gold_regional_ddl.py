"""
Testes de pipelines/ingestion/gold_regional/ddl.py — Gate 6A.
...
from pipelines.ingestion.gold_regional import ddl
...
def test_parse_ddl_statements_arquivo_real_gera_apenas_statements_esperados():
    ...
    assert "gold.marketplace_region_daily" in create_tables[0]
    comments = [s for s in statements if s.upper().startswith("COMMENT ON")]
    assert len(comments) >= 5  # tabela + colunas de cobertura


def test_default_ddl_path_e_o_arquivo_operacional_nao_o_draft():
    assert ddl.DEFAULT_DDL_PATH.name == "marketplace_region_daily_ddl.sql"


def test_ddl_operacional_nao_usa_if_not_exists():
    """Regressao critica: para a PRIMEIRA aplicacao, IF NOT EXISTS mascara
    uma aplicacao parcial anterior (tabela criada mas carga anterior
    falhou, schema pode estar divergente). O arquivo operacional precisa
    falhar alto (relation already exists) em vez de dar no-op silencioso —
    o arquivo _draft.sql (historico, nao mais executado) pode continuar
    usando IF NOT EXISTS."""
    text = ddl.DEFAULT_DDL_PATH.read_text(encoding="utf-8")
    statements = ddl.parse_ddl_statements(text)
    creates = [s for s in statements if s.upper().startswith("CREATE ")]
    assert len(creates) >= 4  # 1 tabela + 3 indices
    for stmt in creates:
        assert "IF NOT EXISTS" not in stmt.upper()
